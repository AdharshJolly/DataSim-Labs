...
_IDENTITY_TYPE_ALIASES = frozenset({"identity", "linked", "person", "user_identity"})
...
class EnhancedDatasetGenerator(DatasetGenerator):
    ...
    def _generate_from_profile_with_stats(
        ...
        # --- Pre-generation assertion (Problem 1) ---
        # If a semantic group with name+email columns exists but those columns
        # were NOT generated, force-run identity batch now.
        groups = getattr(profile, "semantic_groups", []) or []
        for group in groups:
            if str(group.get("type", "")).lower() not in _IDENTITY_TYPE_ALIASES:
                continue
            group_cols = [str(c) for c in group.get("columns", []) if str(c) in all_columns]
            missing = [c for c in group_cols if c not in semantic_group_columns]
            if missing:
                logger.warning(
                    "Pre-generation assertion: semantic group columns %s were not generated "
                    "by _apply_semantic_groups — force-running identity batch.",
                    missing,
                )
                identity_data = generate_identity_batch(
                    ...
                    email_context=group.get("email_context", "personal"),
                    unique=True,
                )
                for col in group_cols:
                    frame[col] = pd.Series(identity_data[col], name=col)
                    semantic_group_columns.add(col)
        ...
        # Skip copula entirely for low-confidence profiles (Problem 7) —
        # fitting a Gaussian copula to ≤15-row correlation matrices is unreliable.
        if copula_cols and hasattr(profile, "correlation_matrices") and not low_confidence:
            ...
            if corr_matrices and "spearman" in corr_matrices:
                ...
                if len(cols) > 0:
                    ...
                    if cond_number > 1e10:
                        logger.warning(
                            "Correlation matrix condition number is very high (%s). Adjusting to PSD.",
                            cond_number,
                        )
        ...
        for col in ordered_columns:
            ...
            if col not in frame.columns:
                frame[col] = self._generate_profile_column(col_profile, row_count, frame=frame)
    ...
    def _apply_semantic_groups(
        ...
        logger.debug(
            "_apply_semantic_groups: %d group(s) in profile.", len(groups),
        )

        for group in groups:
            group_type = str(group.get("type", "")).lower()
            if group_type not in _IDENTITY_TYPE_ALIASES:
                logger.debug(
                    "_apply_semantic_groups: skipping group type '%s' (not identity).",
                    group_type,
                )
                continue
            ...
            logger.info(
                "_apply_semantic_groups: processing identity group — columns=%s, "
                "column_type_map=%s",
                group_columns,
                group.get("column_type_map"),
            )
        ...
        # --- Post-verification (Problem 1) ---
        # Ensure every column that appeared in any group has data in the frame.
        for group in groups:
            group_type = str(group.get("type", "")).lower()
            if group_type not in _IDENTITY_TYPE_ALIASES:
                continue
            for col in group.get("columns", []):
                col_str = str(col)
                if col_str in all_columns and col_str not in generated_columns:
                    logger.warning(
                        "_apply_semantic_groups: column '%s' was in identity group but "
                        "not generated — will be handled by pre-generation assertion.",
                        col_str,
                    )
    ...
    def _generate_profile_column(
        self, col_profile: Dict[str, Any], row_count: int, *, frame: pd.DataFrame | None = None,
    ) -> pd.Series:
        ...
        if data_type == "semantic":
            if semantic_type == "email":
                # If a name column already exists in the frame, derive emails from names.
                if frame is not None:
                    for existing_col in frame.columns:
                        existing_type = detect_semantic_type(existing_col)
                        if existing_type in ("name", "first_name") and existing_col in frame.columns:
                            return generate_email_from_name(
                                col_name=name,
                                names=frame[existing_col],
                                rng=self.rng,
                            )
                return generate_email(name, row_count, self.faker)
            if semantic_type == "name":
